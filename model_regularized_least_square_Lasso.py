...
# Import LassoCV for pure L1 regularization
...
# --- Model Training and Evaluation (LassoCV - Pure L1) ---
...
# max_iter is increased for convergence on large feature sets
model_lasso = LassoCV(alphas=alphas, cv=10, random_state=42, n_jobs=-1, max_iter=10000)
model_lasso.fit(X_train, y_train)

print("--- LassoCV (L1 Regularization) Results ---")
print(f"Best alpha found: {model_lasso.alpha_}")

# Make predictions on the validation set and calculate error
val_predictions = model_lasso.predict(X_val)
...
test_predictions = model_lasso.predict(X_test_scaled)

# Create the submission file
submission_df = pd.DataFrame({'Id': test_ids, 'HotelValue': test_predictions})
submission_df.to_csv('submission_lasso_l1.csv', index=False)

print("Submission file 'submission_lasso_l1.csv' created successfully.")
