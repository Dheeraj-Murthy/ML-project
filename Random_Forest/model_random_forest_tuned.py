...
def run_tuning_and_prediction():
    ...
    try:
        ...
        print(
            f"ERROR: Ensure train.csv and test.csv are in the {DATA_PATH} directory.")
        return
    ...
    # Keep test raw data for preprocessing
    ...
    # Add missing columns (all zeros) to the training set (less common, but
    # safe)
    ...
    print(
        f"Data ready. Training data shape: {
            X_scaled.shape}, Test data shape: {
            X_test_scaled.shape}")
    ...
    print(
        f"Validation Mean Squared Error (Log-transformed, Best Model): {val_mse:.4f}")
